...
var dashboard_query = "SELECT distinct CM_CP_ID,CM_CP_NAME,CM_ULT_PARNT_NAME,CM_CRNT_OBLI_RATING,CM_PREV_OBLI_RATING,CM_PREV_CLTRL_RATING,CM_CRNT_CLTRL_RATING,CM_CRNT_MR_IR,CM_PREV_MR_IR,CM_LAST_REFRESH_TIME,AD_SCORE AS AD_SCORE, AD_SCORE_UPDTIME AS AD_SCORE_UPDTIME FROM COUNTERPARTY_MASTER,ANALYTICAL_DETAILS WHERE CM_CRNT_OBLI_RATING = AD_SCORE ORDER BY AD_SCORE DESC";
...
var tagcloud_query = "select text,weight,orientation from tag_cloud where CP_ID = 'C27'";
...
var fetchBootstrapMainSourceList = function(conn,queryArgs,res){
	...
	conn.query(collateral_Query_max,queryArgs,function(err, rows) {
		...
		pgdata.CR_CRNT_CLTRL_RATING_MAX = IRMax[0].CR_CRNT_CLTRL_RATING;
